...
run: run_c run_cs run_cpp run_java run_python2 run_python3 run_ruby
...
clean:
	@rm -rf __pycache__ *.BAK out.txt diff.txt *.class *.pyc $(FILE_NAME)_cpp $(FILE_NAME)_c $(FILE_NAME)_cs

clean_all:
	find -L . \( -name "__pycache__" -or -name "*.BAK" -or -name "out.txt" -or -name "diff.txt" -or -name "*.pyc" -or -name "*.class" -or -name "$(FILE_NAME)_cpp" -or -name "$(FILE_NAME)_c" -or -name "$(FILE_NAME)_cs" \) -exec rm -rf {} \;
