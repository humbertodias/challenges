...
vector<PT> CLIntersect(PT a, PT b, PT c) {
  ...
  if (dot(_b-_a, ret[0]-_a) < 0) ret.clear();
  return ret;
}
...
int main() {
  ...
  while (scanf("%lf%lf%lf", &X, &Y, &R) == 3 && R > 0) {
    ...
    if (cross(Center - Cannon, p1 - Cannon)*cross(Center - Cannon, p2 - Cannon) > 0) {
      ...
      //printf("> %d %d\n", v1.size(), v2.size());
      if (v2.size() < 2) printf("0.0\n");
      else {
        if (v1.size() < 2) printf("%.1lf\n", areaCircleSeg(Center,v2[0],v2[1],R));
      ...
      if (v1.size() < 2 && v2.size() < 2) {
        if (dot(RotateCCW(eixoX, A), Center - Cannon) < 0) printf("0.0\n");
        else printf("%.1lf\n", PI*R*R);
      } else if (v1.size() < 2) {
        if (dot(RotateCCW(eixoX, A), Center - Cannon) < 0) printf("%.1lf\n", areaCircleSeg(Center,v2[0],v2[1],R));
