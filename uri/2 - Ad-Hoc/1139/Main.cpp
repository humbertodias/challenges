...
long long int area(int p, int MINR, int MAXR, int MAXC){
    int GC = 0, length, start, xact = 0, yact = 0;
    ...
    return max(0LL, ret - GC);
}
int bin_search(int LIM, long long int key, int type, int p = 0, int l = 0){
    int mid, u, v;
    long long A;
    for(u = 1, v = LIM; u < v; ){
        mid = (u + v) >> 1;
